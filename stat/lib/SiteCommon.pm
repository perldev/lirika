...
###if there'nt any operations		
	...
	if($concl_sum<0)
	{
		...
		push @$rows,{ct_ex_comis_type=>'transaction',ct_date=>format_date($proto->{beg_calc_date}),
				...
				result_amnt=>$tm_sum,
				ct_comment=>qq[Начисление кредита за $days дня],
				col_status=>'no'};
...
sub get_account_name
{
    ...
    "$r->{a_name} (id#$r->{a_id}) [$r->{a_class}] $r->{a_uah} ГРН, $r->{a_usd} USD, $r->{a_eur} EUR",eur=>$r->{a_eur},usd=>$r->{a_usd},uah=>$r->{a_uah},ac_id=>$r->{ac_id}
    };
