...
sub sum
{
	...
	unless($proto->{sums}->{$row->{ct_date}})
	{
		...
		push @$array,{ct_ex_comis_type=>'concl',prev_date=>format_date($prev_row),ct_date=>format_date($row->{ct_date}),
 			      	UAH=>$proto->{sums}->{ $prev_row }->{UAH},
 			      	USD=>$proto->{sums}->{ $prev_row }->{USD},
				EUR=>$proto->{sums}->{ $prev_row }->{EUR},
				UAH_FORMAT=>format_float($proto->{sums}->{ $prev_row }->{'UAH'}),
...
sub get_account_name
{
    ...
    "$r->{a_name} (id#$r->{a_id}) [$r->{a_class}] $r->{a_uah} ГРН, $r->{a_usd} USD, $r->{a_eur} EUR",eur=>$r->{a_eur},usd=>$r->{a_usd},uah=>$r->{a_uah},ac_id=>$r->{ac_id},a_incom_id=>$r->{a_incom_id}
    };
...
sub get_firm_name
{
	...
	"$r->{f_name} (id#$r->{f_id}) ".format_float($r->{f_uah}).' ГРН, '.format_float($r->{f_usd}).' USD,'
...
sub get_service_percent_client{
    ...
    return $dbh->selectrow_array(q[
            ...
            WHERE cs_aid=?  AND cs_fsid=? ],undef,$id,$service_id);    
