...
sub get_grad
{
  ...
  {
     push @result,{y=>ceil(100*(($item*$i)*$pow))/100,from_x=>0,to_x=>$size,
                  from_y=>20*$i,to_y=>20*$i};
  }
  return {yy=>\@result,max_y_value=>$item*11*$pow};
}
...
{
  my $stm=shift;
  my $ref=$dbh->selectall_arrayref($stm);
...
for(my $r=1;$r<=$month{$month};$r++)
{
  ...
  for(my $i=1;$i<=$month{$month};$i++)
  {
    ...
    return {'stat'=>\@result,xx=>\@xx,yy=>$grad->{yy},max_y_value=>$maxy};
