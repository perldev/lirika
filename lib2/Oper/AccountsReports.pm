...
sub mari{
    my $self=shift;
    my $tmpl=$self->load_tmpl('reports_marine.html');
...
sub send_balance
{
	...
	if(!$email&&$self->{non_login})
	{
		print "there isn't mail for a_id $id \n";
			
	}elsif(!$email&&!$own_email)
	{
		die "Вы не задали электронный адрес для отправки  \n";
	}
...
#	print "rar a -hp$passwd $file1 $file2 $file  ";
	...
	unless($own_email)
	{
		...
		map {send_mail({file=>"$file1",mail_to=>$_} ) } @mails ;

	}else
	{
		send_mail({file=>"$file1",mail_to=>$own_email} );
	}
...
sub built_excel
{
	...
	($mines,$_->{rate})=$self->calculate_exchange(0,$_->{rate},$_->{orig__ct_currency},$_->{e_currency2});
    ...
    my $min_date=$dbh->selectall_arrayref(qq[SELECT YEAR('$NEW_DOC_FROM_DATE'),
       ...
       $docs->{$NEW_DOC_FROM_DATE}={d=>$NEW_DOC_FROM_DATE,s=>$debt, comis=>$percent*($debt/100)};
        ...
        my $money_main=$dbh->selectall_arrayref(qq[SELECT  
                                        ...
                                        ct_amnt as s,ct_comment,ct_fid
                                        FROM cashier_transactions
											                                        ...
											                                        FROM cashier_transactions                                                                                            
																                                        WHERE ct_aid=$parent_aid                                                                                             
																					                                        AND ct_currency='$RESIDENT_CURRENCY'                                                                                 
																										                                        AND ct_fid>0 AND ct_amnt>0                                                                                           
																															                                        AND  ct_status='processed'  AND ct_date>'$NEW_DOC_FROM_DATE'                                                         
=cut																																				                                        ORDER BY ct_date ];

         my $money_transit=$dbh->selectall_arrayref(qq[SELECT  
                                        ct_date as d,CONCAT(MONTH(ct_date),'.',YEAR(ct_date)) as ds ,
                                        ct_amnt as s,(ct_amnt/100)*ct_comis_percent as sum_perc,ct_comment,ct_fid
                                        FROM cashier_transactions 
                                        WHERE
...
#getting the min date for generating the date
        ...
        foreach my $date (@{ $ar }){
          ...
          if($date eq $money_transit->[$cur2]->[0]){
                ...
                for($j=$cur2;$j<$size2&&$date eq $money_transit->[$j]->[0];$j++){
