...
sub edit
{
	...
	$params{'do'}='edit';
  	...
  	if($action eq 'apply'&&!$param)
  	{
        ...
        return $self->error(q[Не может быть дополнительной и приходной комиссии ])    if($params{ct_ext_commission_perc}&&$params{ct_ext_commission});
...
sub proto_add_edit_trigger{
    ...
    if(!$ra->{a_incom_id}&&$params->{ct_ext_commission_perc}){
	...
	$params->{ct_comment}",
     ...
     if($params->{ct_ext_commission}||$params->{ct_ext_commission_perc})
     {
        my $to_id=$comis_aid;
		$to_id=$ra->{a_incom_id}  if($params->{ct_ext_commission_perc});
	...
	if((`cashier_transactions`.`ct_eid` is not null),
	`exchange_view`.`e_amnt1`,(`cashier_transactions`.`ct_amnt` -  abs(IF(ct_ext_commission,ct_ext_commission,0))-
