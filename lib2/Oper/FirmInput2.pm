...
{       
    ...
    'fields'=>[
        ...
        {
	    'field'=>"ct_fsid", "title"=>"Услуга", filter=>'=',"category"=>"firm_services", "may_null"=>1,
        ...
        {'field'=>"ct_comis_percent", "title"=>"Комиссия% ",java_script_action=>'onkeyUp="change_commission()"',            
            no_base=>0 },
...
{
   my $self = shift;
    $proto->{fields}->[5]->{type}='select';
 $proto->{fields}->[5]->{titles}=get_services();
...
##protections from double clicking
	...
	my $r = $dbh->selectrow_hashref(q[SELECT * FROM cashier_transactions WHERE ct_status='processing' AND ct_id=?],undef,$id);
   ...
   return 1;
	...
	(`cashier_transactions`.`ct_amnt` - abs(IF(ct_ext_commission,ct_ext_commission,0))- ((`cashier_transactions`.`ct_comis_percent` * `cashier_transactions`.`ct_amnt`) / 100))) AS `result_amnt`,
...
{	
	...
	die "Вы не выбрали карточку \n " unless($dbh->selectrow_array('SELECT a_id FROM accounts WHERE a_id=?',undef,$ct_aid));
       ...
       die "Процент комиссии слишком большой\n" if(abs($per_exch)>2*$ETALON_VALUE_COMIS||abs($per)>2*$ETALON_VALUE_COMIS);
	...
	foreach(@ct_id)
	{
		...
		$params->{proto}->{fields}->[1]->{expr},$per,$ct_ts2,$_);
