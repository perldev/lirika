...
sub save_state
{
	...
	$ref->{firms}=$firms;
	my $serialized = freeze($ref);    
	return $serialized;
}
sub restore_state
{
	my %params = %{thaw($dbh->selectrow_array(q[SELECT cr_system_state 
...
sub get_kcards_without
{
	...
	accounts_view,owners_without WHERE a_id=ow_aid AND  1],'a_id');
...
sub get_rates
{
    ...
    my $ref=$dbh->selectrow_hashref(q[SELECT hr_domi,hr_rate_street,hr_rate_cross 
    FROM header_rates  ORDER BY 
    hr_id DESC LIMIT 1]);
    my $uah=$ref->{hr_domi};
...
sub get_permanent_cards_without_cats
{
	...
	NOT IN  ($non_usual_class) 
	AND $count_param>0 
	AND c_id=a_class AND a_acid=ac_id AND a_status!='deleted' AND ac_id=$cat_id
	AND a_id>1
	GROUP BY a_id ORDER BY a_name ASC],'a_id');
	my @plus_cards;
	...
	WHERE c_name NOT IN  ($non_usual_class) AND $count_param<=0 AND 
	c_id=a_class 	AND a_id>1 AND a_acid=ac_id AND a_status!='deleted' AND ac_id=$cat_id
	  GROUP BY a_id  ORDER BY a_name ASC],'a_id');
	my @mines_cards;
	@keys = sort { $r->{$a}->{a_name} cmp $r->{$b}->{a_name} } keys %$r;
	my $size__1=@keys;
...
##get static cards
	...
	my $echs=get_exchanges();
##get static cards
	my $res=get_cats_simple();
	shift @$res;##delete SYSTEM category
	my @common_result;
