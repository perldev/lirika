...
##get through all right and setting a name for it
   ...
   my @oper_tabs;
   map{ $self->{tabs}->{$_}=$tabs->{$_}->{title} } keys %$rights;
...
sub add_trans_firm
{
	...
	($ref->{f_id1},$ref->{f_name1})=$dbh->selectrow_array(q[SELECT f_id,f_name FROM firms WHERE f_id=? ],undef, $ref->{f_id1});
         ...
         die "Такая фирма не зарегистрирована \n" unless($ref->{f_id1});

	($ref->{f_id2},$ref->{f_name2})=$dbh->selectrow_array(q[SELECT f_id,f_name FROM firms WHERE f_id=? ],undef,$ref->{f_id2});

        die "Такая фирма не зарегистрирована"     unless($ref->{f_id2});

	$ref->{comment}="Транзит   : С $ref->{f_name1} на  $ref->{f_name2} $ref->{amnt} $ref->{currency}"  	unless($ref->{comment});

        my $field=$avail_currency_firms->{$ref->{currency}};
        $ref->{account_bill}=$field;
	die "Такая валюта не поддержуется \n"  unless($field);
...
# but do it in an eval{} as it may also fail
                eval { $dbh->rollback };
		my $r=$dbh->errstr;
                 die "Обратитесь к разработчику: code 4 ($qwe)\n"; 
...
sub add_trans
{
	...
	$ref->{o_login}=$self->{o_login};
        die "Такой пользователь   не зарегистрирован \n" unless($ref->{t_name1});
...
#my $a2=$dbh->selectrow_array(q[SELECT a_id FROM accounts WHERE a_id=? ],undef,$ref->{t_name2});
	...
	$ref->{t_amnt}=~s/[ \"\'\\]//g;
         ...
         die "Неправильный формат числа $ref->{t_amnt}\n" unless($ref->{t_amnt}=~/[\-]{0,1}\d+\.{0,1}\d{0,2}/);
...
sub do_trans
{
	  ...
	  @$ref{qw( t_aid1 t_aid2 t_amnt t_currency  t_comment t_oid t_status sum_before1 sum_before2 del_status)});
          die " Обратитесь к разработчику: code 2\n" unless($res eq 1);   
...
sub add_exc
{
        ...
        die " Курс не задан" unless($param->{'rate'});
        require POSIX;
...
sub get_user_by_session
{
	...
	if($ENV{REMOTE_ADDR}  ne '127.0.0.1')
	{
	 ...
	 ($os_oid,$sub)=$dbh->selectrow_array(qq[SELECT    os_oid,                                                                                       
	                 timestampdiff(minute,current_timestamp,os_expired)                                                                                          
	              FROM oper_sessions WHERE  os_session=? AND os_status='active'  AND os_expired>current_timestamp],undef,$session);
	...
	###caching the session data
		...
		my ($str,$o_login,$o_greeding)=$dbh->selectrow_array(q[SELECT o_session_data,o_login,o_greeding 
...
##if  we defind a sub of working with te record
	...
	$result_params{timer}=$TIMER->stop;
...
sub export_excel
{
    ...
    unlink("../data/excel/$file_server.xls");
    $self->built_excel(\@rows,">../data/excel/$file_server.xls",\@headers);
    return $file;
...
sub exc_list
{
        ...
        *
        FROM exchange_view   WHERE   $ref->{'filter'}  ORDER BY e_id DESC LIMIT].qq[ $ref->{'page'},$ref->{'how'}]);
         ...
         my %types=('cash'=>'пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ','cashless'=>'пїЅпїЅпїЅпїЅпїЅпїЅ.',auto=>'пїЅпїЅпїЅпїЅ');
        my @res;
        $sth->execute();
...
##filters
		...
		{         
			...
			$self->query->param($row->{field},$val);
			    return $self->error("$prefix не соответсвует формату числа!!")  unless ($val =~ /^\s*[+-]?\d+\.?\d*\s*$/);
			$val += 0;
			return error $self,"$prefix меньше 0" if($val <= 0);
		...
		{
			return  error $self,"$prefix не зарегистрировано в системе !" unless($avail_currency->{$val});

		}
        elsif($row->{etalon}&&abs($val)>(2*$row->{etalon}))
        {
            return error $self, "$prefix слишком отличается от эталона  \n";
		...
		elsif($row->{uniq})
		{
			...
			if($id&&$dbh->selectrow_array("SELECT $row->{field} FROM  $proto->{table} WHERE $row->{field}=? 
						AND  ",undef,$val))
			{
				return error $self,"$prefix должно быть уникальным  !!! \n";
...
# 
		...
		{
	return error $self, "Поле комментариев должно быть не пустым \n" unless($self->query->param('ct_comment'));
...
##for checking objections of common identification
	...
	{
		die " Вы не выбрали ни одной записи \n";
	}
	my $count=@res;
	...
	$self->{tpl_vars}->{sum}=$sum;
   	...
   	$self->{tpl_vars}->{page_title}='Групповое добавление';
...
##for checking objections of common identification
	...
	{	
		...
		$r->{comission}=$ct_comis_percent*($r->{ct_amnt}/100)+$r->{ext_comission};
	...
	$self->{tpl_vars}->{my_ct_comis_percent}=$ct_comis_percent;
	...
	$self->{tpl_vars}->{page_title}='Подтверждение группового добавления';
