...
sub get_head_rates
{
	my $ref=$dbh->selectrow_hashref(q[SELECT hr_domi as usd_uah_mb,hr_rate_mb as usd_uah,hr_rate_street as cusd_uah,hr_rate_cross as eur_usd FROM header_rates 
              ...
              #      $r->{compare}=$r->{$_};
               #     $r->{$_}=andrey_float_format($r->{$_});
                ...
                #        $r->{compare}=$r->{$_};
                    ...
                    # $val = 0-$val if($key->{op} eq '-');
                        $r->{$name}=andrey_float_format($val);
                   #     $r->{id} = $r->{ $_->{"id_field"} };
                        return  $r->{$name};
                    };
                    $hash->{$_->{field}}=$ref;
...
sub exc_list
{
        ...
        *
        FROM exchange_view   WHERE   $ref->{'filter'}  ORDER BY e_id DESC LIMIT].qq[ $ref->{'page'},$ref->{'how'}]);
         ...
         my %types=('cash'=>'наличный','cashless'=>'безнал.',auto=>'авто');
        my @res;
        $sth->execute();
...
##for checking objections of common identification
	...
	$self->{tpl_vars}->{account_title}=get_account_name($ct_aid)->{ext_info};
