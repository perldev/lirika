...
sub add_trans_firm{
	...
	($ref->{f_id1},$ref->{f_name1})=$dbh->selectrow_array(q[SELECT f_id,f_name FROM firms WHERE f_id=? ],undef, $ref->{f_id1});
        ...
        die "Фирма отправитель не зарегистрирована в системе \n" unless($ref->{f_id1});

	($ref->{f_id2},$ref->{f_name2})=$dbh->selectrow_array(q[SELECT f_id,f_name FROM firms WHERE f_id=? ],undef,$ref->{f_id2});

        die "Фирма получатель не зарегистрирована в системе"   unless($ref->{f_id2});

	$ref->{comment}="Транзит между фирмами: с $ref->{f_name1} на  $ref->{f_name2} $ref->{amnt} $ref->{currency}" 	unless($ref->{comment});
	...
	die "Сумма не соответствует формату числа\n" unless($ref->{amnt});
...
# but do it in an eval{} as it may also fail
                eval { $dbh->rollback };
		my $r=$dbh->errstr;
                die "Внутрення ошибка разработчика: code 4 ($qwe)\n"; 
...
#my $a2=$dbh->selectrow_array(q[SELECT a_id FROM accounts WHERE a_id=? ],undef,$ref->{t_name2});
	...
	$ref->{t_aid2}=$ref->{t_name2};
        ...
        die "Такого счета не существует \n" unless($field);
        
	$ref->{t_amnt}=~s/[,]/\./g;
	$ref->{t_amnt}=~s/[ \"\'\\]//g;
	$ref->{t_amnt}=$ref->{t_amnt}*1;
        die "Сумма не соответствует формату\n" unless($ref->{t_amnt});
...
sub do_trans
{
        ...
        die "Внутренняя ошибка обратитесь к разработчеку : code 1\n" unless($res eq 1);
	  ...
	  @$ref{qw( t_aid1 t_aid2 t_amnt t_currency  t_comment t_oid t_status sum_before1 sum_before2 del_status)});
          die " Внутренняя ошибка обратитесь к разработчеку: code 2\n" unless($res eq 1);   
	  ...
	  if($ref->{t_status} eq 'system')
	  {
        ...
        die "Внутренняя ошибка обратитесь к разработчеку : code 3\n" unless($res eq 1);
...
sub add_exc
{
        ...
        require POSIX;
        die "Валюта обмена не зарегистрирована в системе \n"	unless($avail_currency->{$param->{e_currency1}});
...
##filters
		...
		{         
			...
			return $self->error("$prefix не соответствует формату числа!!") unless ($val =~ /^\s*[+-]?\d+\.?\d*\s*$/);
			$val += 0;
			return error $self,"$prefix не может быть отрицательным" if($val <= 0);
		...
		{
			return  error $self,"$prefix такая валюта не зарегистрирована !" unless($avail_currency->{$val});
...
# 
		...
		{
	return error $self, "Заполните поле комментарийев \n" unless($self->query->param('ct_comment'));
