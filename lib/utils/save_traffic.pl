...
my $common_aids_in=$dbh->selectall_hashref(qq[SELECT a_id,a_name,sum(ct_amnt) as s  
					      FROM cashier_transactions,firms,accounts 
					      WHERE ct_aid=a_id  AND  ct_fid=f_id   
					     AND ct_fid IN ($str) AND ct_status IN ('created','processed') 
					    AND ct_amnt>0 AND MONTH(ct_date)=$params[1]  AND YEAR(ct_date)=YEAR(current_timestamp) 
