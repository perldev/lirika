...
sub get_right
{
    ...
    $proto={
        ...
        'page_title'=>" Ввод наличных $kassa_title",
        ...
        'fields'=>[
            ...
            {'field'=>"ct_amnt", "title"=>"Сумма", 'filter'=>"eq", "positive"=>1,java_script_action=>"onkeyup='amnt_pressed()'",
            java_script=>"
...
sub   add_many
{
	...
	my $output='';
	$self->{tpl_vars}->{page_title}="Добавление заявок на приход $proto->{cash_title}";
...
sub add_many_confirm
{
	...
	$self->{tpl_vars}->{page_title}="Подтверждение  заявок на приход $proto->{cash_title}";
	$tmpl->process($self->{tpl_file}, $self->{tpl_vars}, \$output) || die $tmpl->error();
		...
		if(defined $params{"ct_aid$i".'name'})
		{
			...
			for(my $j=0;$j<$size2;$j++)
			{
				...
				$conv_currency->{$params{"ct_currency$i"}->[$j]})
				{
					...
					$sql.="($kassa_id,$ct_aid,$ct_currency,$ct_amnt,$ct_comment,$ct_oid,NOW(),NOW()),";	
				}
...
sub back
{
	...
	$proto->{page_title}="Возврат денег $proto->{cash_title}";
...
sub proto_add_edit_trigger{
  ...
  if($params->{step} eq 'before'){
    ...
    if($id)
    {
        $percent=$dbh->selectrow_array(qq[SELECT ct_comis_percent FROM cashier_transactions 
        WHERE ct_id=? AND ct_fid=$kassa_id],undef,$id);
   ...
   return 1;   
	...
	$dbh->do($params->{sql});
		...
		$dbh->do(qq[UPDATE cashier_transactions SET ct_status='returned' WHERE ct_id=?],undef,$id);
