...
{       
    ...
    my $kassa_title;
    $co_id=undef;
    ($kassa_id,$kassa_title,$co_id, $ex)=$dbh->selectrow_array(q[SELECT co_aid,co_title,co_id, co_script_ex
                                      FROM cash_offices 
    ...
    if($self->{currencies}){
     ...
     $proto={
        ...
        'page_title'=>"Заявки на вывод наличных $proto->{cash_title}",
        ...
        //              var common_sum=get_element('common_sum',1);
                        ...
                        var comis_amnt = percent*(common_sum/(1-percent/100)/100) 
                        set_element('commission', comis_amnt);
                        set_element('ct_amnt',common_sum - comis_amnt);
...
#             
	...
	$proto->{cash_title}=$kassa_title;
        ...
        WHERE DATE(cob_date)=DATE(current_timestamp) AND cob_coid=? AND cob_block='yes'],undef,$co_id);
        return 'cash_out_'.$self->{cash};
...
sub   add_many
{
    ...
    $self->{tpl_vars}->{services}=&get_cash_services($proto->{cash});
	$self->{tpl_vars}->{page_title}="Добавление заявок на расход $proto->{cash_title}";
...
sub add_many_confirm
{
	...
	$self->{tpl_vars}->{page_title}="Подтверждение  заявок на расход $proto->{cash_title}";
	$tmpl->process($self->{tpl_file}, $self->{tpl_vars}, \$output) || die $tmpl->error();
	...
	for($i=1;$i<=$size;$i++){
		...
		if(defined $params{"ct_aid$i".'name'})
		{
            ...
            my $o=$dbh->selectrow_array(q[SELECT cob_id
                                            ...
                                            cob_block='yes'
                                            AND (
                                            DATE(cob_date)=? OR DATE(cob_date)>?) AND cob_coid=?
                                            ],undef,$params{"ct_date$i"},$params{"ct_date$i"},$co_id);
			...
			$size2=@{ $params{"ct_amnt$i"} } if($params{"ct_amnt$i"});
			for(my $j=0;$j<$size2;$j++)
			{
...
#                     $ct_date=$params{"ct_date$i"};
					$sql.="($kassa_id,$ct_aid,$ct_currency,$ct_amnt,$ct_comis,$ct_comment,$ct_oid,NOW(),$ct_date),";	
				}
...
sub back
{
	...
	unless($ref){
			$self->header_type('redirect');
            return  $self->header_add(-url=>$ENV{HTTP_REFERER});
	}
    ...
    my $const= int(($ref->{ct_ext_commission}/($ref->{ct_amnt}))*10000)/10000  or 0 ;
    
	$const=int((($const*100)/(1+$const)+$STAND_MIS)*1000)/1000 if($const);
...
#         die "У данной программы не обозначенна в настройках приходная \n"    if(!$income_card&&$self->query->param('ct_ext_commission')>0);
   ...
   my $o=$dbh->selectrow_array(q[SELECT cob_id FROM cashier_out_block 
    WHERE cob_block='yes' AND cob_coid=? AND (DATE(cob_date)=? OR DATE(cob_date)>?)  ],undef,$co_id,$self->query->param('ct_date'),$self->query->param('ct_date'));
...
#      }
   ...
   return 1;   
	...
	{
		$dbh->do(qq[UPDATE accounts_reports_table SET ct_status='returned' WHERE 
		ct_ex_comis_type='input' AND  ct_fid=$kassa_id AND ct_id=?],undef,$id);
