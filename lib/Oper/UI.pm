...
use base q[CGIBase];
use SiteCommon qw(get_accounts paging trim @months $avail_currency &time_filter );
use SiteConfig;
use SiteDB;
...
[
		 ...
		 {'field'=>"t_ts_mysql", "title"=>"Дата", 'filter'=>"time"},
  		 #{'field'=>"t_amnt", "title"=>"Сумма", 'filter'=>"="},
   		 {'field'=>"t_currency", "title"=>"Валюта", "type"=>"select", "titles"=>[
       			{'value'=>"UAH", 'title'=>"UAH"},
       			{'value'=>"USD", 'title'=>"USD"},
       			{'value'=>"EUR", 'title'=>"EUR"}]
    		, 'filter'=>"="
...
sub trans_list
{
	...
	if($_POST->{'a_id'})
	{
		...
		$self->{tpl_vars}->{trans_list}->{a_name}=$dbh->selectrow_array(q[SELECT  a_name FROM  accounts WHERE a_id=?],undef,$_POST->{a_id});
...
##ending from oleg
	...
	#for working with this runmode
	 ...
	 my $ref=get_trans_list({color=>$_POST->{'color'},filter=>$filter,page=>$page,how=>$how});
	...
	foreach(@{$ref->{transes}})
	{
	 ...
	 $self->{tpl_vars}->{title}='Список транзакций';
