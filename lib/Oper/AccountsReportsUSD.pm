...
sub plug_in{
    ...
    my $hash=$dbh->selectall_hashref(q[SELECT cr_xml_detailes,DATE(cr_ts) as date,cr_id,
                                       cr_delta_usd as usd,cr_delta_eur as eur,cr_delta_uah as uah 
                   			           FROM reports_without  
                                       WHERE cr_status='created' AND cr_id<71 AND cr_id<42],'cr_id');
    ...
    foreach(keys %$hash){
        ...
        my @tids;
        my %params = %{thaw($tmp->{cr_xml_detailes})};
        my $of_spents= $params{master_cards}->[1];
        my %delta=( 'USD'=>$tmp->{usd}, 'UAH'=>$tmp->{uah}, 'EUR'=>$tmp->{eur} );
        
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$delta{USD}*0.25,
            t_comment=>'Распределение доходов 0.25',
            t_aid1=>-8,
            t_aid2=>669,
            ts=>$tmp->{date},
            t_currency =>'USD'
        ...
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$of_spents->{'USD'}*0.25,
            t_comment=>'Распределение трат 0.25',
            t_aid1=>-8,
            t_aid2=>669,
            ts=>$tmp->{date},
            t_currency =>'USD'
        ...
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$delta{USD}*0.5,
            t_comment=>'Распределение доходов 0.5',
            t_aid1=>-8,
            t_aid2=>668,
            ts=>$tmp->{date},
            t_currency =>'USD'
        ...
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$of_spents->{'USD'}*0.5,
            t_comment=>'Распределение трат 0.5',
            t_aid1=>-8,
            t_aid2=>668,
            ts=>$tmp->{date},
            t_currency =>'USD'
        ...
        my $str=join(',',@tids);
        $dbh->do(qq[UPDATE reports_without SET cr_tids='$str' WHERE cr_id=?],undef,$tmp->{cr_id});



    }

    $hash=$dbh->selectall_hashref(q[SELECT cr_xml_detailes,DATE(cr_ts) as date,
                                    cr_id,cr_delta_usd as usd,cr_delta_eur as eur,cr_delta_uah as uah 
                                    FROM reports_without  
                                    WHERE cr_status='created' AND cr_id<71 AND cr_id>=42],'cr_id');
    ...
    foreach(keys %$hash){
        ...
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$delta{USD}*0.33,
            t_comment=>'Распределение доходов 0.33',
            t_aid1=>-8,
            t_aid2=>669,
            ts=>$tmp->{date},
            t_currency =>'USD'
        ...
        push @tids,Oper::Transfers::add_transfer($self,{
            t_amnt=>$of_spents->{'USD'}*0.33,
            t_comment=>'Распределение трат 0.33',
            t_aid1=>-8,
            t_aid2=>669,
            ts=>$tmp->{date},
            t_currency =>'USD'
    ...
    my $ref=$dbh->selectall_arrayref(q[SELECT t_id FROM transfers WHERE o_id=2 AND DATE(t_date)='2010-08-04']);
    my @tids;
    foreach(@$ref){
        my $new=$self->del_trans($_->[0]);
        push @tids,$new;
        $dbh->do(q[UPDATE accounts_transfers SET at_tid_back=?,at_status='deleted' WHERE at_tid=?],undef,$new,$_->[0]);
        push @tids,$_->[0];
    }
 
    my $str=join(",",@tids);
    $dbh->do(qq[UPDATE accounts_reports_table_archive SET ct_status='deleted' 
                WHERE 
                ct_ex_comis_type='transaction' AND ct_id IN($str)]);
