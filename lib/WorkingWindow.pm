...
#   return undef if($res ne 1);
    ...
    $dbh->do(q[UPDATE accounts_num_records SET anr_num=(SELECT count(*) 
               FROM accounts_reports_table WHERE ct_aid=?) WHERE anr_aid=?],undef,$aid,$aid);
...
###it so bad that this request is here ((
    ...
    $dbh->do(q[UPDATE accounts_num_records SET anr_num=(SELECT count(*) 
               ...
               WHERE ct_aid=?) WHERE anr_aid=?],undef,$id,$id);
