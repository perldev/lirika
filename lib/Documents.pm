...
sub get_out_firms_and_okpo
{
	    my $some=shift;
	   my $sql = qq[
      	 SELECT of_id,of_name,of_okpo FROM out_firms
	   ...
	   #     push @titles_okpo, {"value"=>$r->{of_id}, "title"=>"$r->{of_okpo}"} if($r->{of_okpo});
       #}
       
       while(my @r = $sth->fetchrow_array)
       {
	        push @titles, {"value"=>$r[0], "title"=>"$r[1] (id#$r[0])"};
	        push @titles_okpo, {"value"=>$r[0], "title"=>"$r[2]"} if($r[2]);
       }
       unshift @titles,{"value"=>'', "title"=>"Новая"} unless($some);

       $sth->finish();
       my @arr = (\@titles_okpo,\@titles);
       return @arr;
    ...
    ##must work for old records
     ...
     $dbh->do(qq[UPDATE accounts_reports_table_archive SET ct_status='deleted' 
      ...
      my $new_id=$self->add_trans(
        ...
        WHERE ct_id IN ($new_id) AND ct_ex_comis_type='transaction']);
        ...
        $pays->{0}={dp_tid=>0};##if pays are empty for sql_d requests
   
        my $p_str=join(q[,],keys %$pays);

       my $sql_d=qq[DELETE LOW_PRIORITY FROM  accounts_reports_table 
                   WHERE ct_id IN ($new_id,$p_str) AND ct_ex_comis_type='transaction'];


        my $ref=$dbh->selectall_hashref(qq[SELECT $flds,concat(ct_id,ct_ex_comis_type,ct_aid) as 'key' 
                                           FROM accounts_reports_table 
                                           WHERE ct_id IN ($new_id,$p_str) AND ct_ex_comis_type='transaction' ],'key');
        
        my @ar;
        my $tmp_s='';
        foreach my $t (keys %$ref){
            my $tmp=$ref->{$t};
            map{$tmp_s.="'$tmp->{$_}',"} @flds; 
            push @ar,"($tmp_s 0)";
            $tmp_s='';
        }
        if(@ar){
...
sub get_payment_
{
	   ...
	   my $dr_ids=$dbh->selectall_hashref(qq[SELECT 
  	    ...
  	    FROM
			...
			AND dr_id=dp_drid    	AND dr_status IN ('created','processed')       	AND dr_aid=a_id  AND dr_id IN ($str) 	GROUP BY dr_id],'dr_id');
...
=cut
        ...
        documents_transactions 
        SET dt_drid=? 
        WHERE dt_aid=? AND dt_currency=? AND dt_ofid=? AND dt_fid=? 
		AND dt_status='created' 
        AND MONTH(dt_date)=MONTH(?) 
        AND YEAR(dt_date)=YEAR(?) AND  dt_drid IS NULL],
		undef,$doc_id,
		$q{'dr_aid'},
		$q{'dr_currency'},
		$q{'dr_ofid_from'},
...
=cut
			...
			$dbh->do(q[INSERT INTO documents_requests(
			dr_comis,
			dr_aid,
			dr_amnt,
			dr_comment,
			...
			my $dr_id=$dbh->selectrow_array(q[SELECT last_insert_id()]);
			my $l_id=0;

			
			$dbh->do(qq[INSERT INTO documents_payments(dp_ctid,dp_drid,dp_tid) VALUES(?,?,?)],undef,$l_id,$dr_id,0);
