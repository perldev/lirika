...
use locale;
use POSIX 'locale_h';
setlocale(&POSIX::LC_ALL,"ru_RU.CP1251");
$ENV{LC_ALL} = 'ru_RU.cp1251';

my $TIMER  = RHP::Timer->new();
$TIMER->start('fizzbin');
open(FL,">>".$MAIL_DIR."/logs2.dat");
my ($sec,$min,$hour) = localtime();
print FL "\nstart in $hour:$sec:$min - work:";
...
sub do_main
{
  ...
  foreach my $oWkS (@{$oBook->{Worksheet}})
  {
    ...
    for (my $iR = $oWkS->{MinRow}+1; defined $oWkS->{MaxRow} && $iR <= $oWkS->{MaxRow} ; $iR++)
    {
    	...
    	if(($excel_row->{sum6} || $excel_row->{sum5} || $excel_row->{sum4} || $excel_row->{sum4} || $excel_row->{sum2} || $excel_row->{sum1}) && ($excel_row->{firm} ne "" || $excel_row->{okpo} ne ""))
    	{
