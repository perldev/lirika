...
function show_change_date_comment(date,obj,ev){
       var scr=getScrollXY();
       $('#return_id').attr({ 'value':obj.id });
       var str=obj.innerHTML ;
       $('#date_comment').attr({ 'value': str } );
       $('#comment_date').attr({ 'value':date } );
       $('#date_comment_form').css({ 'right':250,'top':ev.clientY+scr[1]+10 } );
       $('#date_comment_form').show('slow');




}
function date_comment_change(){

        var date=$('#comment_date').attr('value');

        var comment=$('#date_сomment').attr('value');

        var aid=$('#aid').attr('value');

        $.post('ajax.cgi',
                'do=change_in_accounts_comment&aid='+aid+'&comment='+comment+'&date='+date,
                function (dat){ 
                    $('#'+$('#return_id').attr('value') ).html( comment );
                    $('#date_comment_form').hide('slow');
...
function event_handle(_form_id,_obj,_id,_typ_e){
        ...
        this.handle=function(){
           ...
           var only=$('#only').attr('value');
           if(!$('#only').attr('checked'))
                only='';
           SendRequest('ajax.cgi','do=change_ts_accounts_report&ts='+val+'&type='+typ_e+'&ct_id='+id+'&only='+only,'POST',NULL_FUNC);
	       vals=val.split("-");
...
function uncheck_row(number)
{
    ...
    var ready=REQ.readyState;
        if(ready==READY_STATE_COMPLETE)
        {
            ...
            var idd=rows[number].cells[0].id;
            var id1=idd.replace(/[^\d]/g,""); 
            var type_1=idd.replace(/[\d]|row/ig,"");
            document.getElementById(type_1+id1).checked=false;
...
function uncheck(type_,id)
{
    ...
    {   
        ...
        {
            number=i;
            color=rows[i].cells[0].style.backgroundColor;
            break;
        }
    ...
    if(ct_aid==617){
        for(var i=0;i<rows.length;i++){
/*            alert(rows[i].cells[0].style.backgroundColor+','+color+','+i);*/
            
            if(rows[i].cells[0].style.backgroundColor==color){


                   var idd=rows[i].cells[0].id;
                   var id1=idd.replace(/[^\d]/g,""); 
                   var type_1=idd.replace(/[\d]|row/ig,"");

                   for(var z=0;z<size;z++){

                        if(concl_array[z]==id1&&concl_type[z]==type_1){

                            concl_array[z]=concl_array[size-1];
                            concl_type[z]=concl_type[size-1];
                            concl_array_whole[z]=concl_array_whole[size-1];
                            concl_array.pop();
                            concl_type.pop();
...
//                     for(var j=0;j<hidden_arries.length;j++){
//                             alert(hidden_arries[j]+",");
//                     }
                   var j=binary_search(hidden_arries,number);
                   delete_from_array(hidden_arries,j);
/*                   alert('ct_aid='+ct_aid+'&do=set_col_no&id='+id1+'&type='+type_1); */
                    SendRequest('ajax.cgi','ct_aid='+ct_aid+'&do=set_col_no&id='+id1+'&type='+type_1,'POST',obj.listener);
            }
